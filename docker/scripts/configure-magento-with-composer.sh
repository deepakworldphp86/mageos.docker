...
source '.env'
  ...
  echo "Downloading Magento installation from ${MAGE_OS_MIRRORS}"
  OMPOSER_MEMORY_LIMIT=-1 composer create-project --repository-url=${MAGE_OS_MIRRORS} ${MAGE_OS_VERSION} /var/www/html/${MAGE_INSTALLATION_FOLDER} 
  ...
  if [[ "${SEARCH_ENGINE}" ]]; then
    install_args+=("--search-engine=$SEARCH_ENGINE")
  fi
  if [[ "${SEARCH_HOST}" ]]; then
    install_args+=("--opensearch-host=$SEARCH_HOST")
  fi
  ...
  echo "Running Magento install: php -d memory_limit=-1 /var/www/html/${MAGE_INSTALLATION_FOLDER}/bin/magento setup:install ${install_args[@]}"
  php -d memory_limit=-1 /var/www/html/"${MAGE_INSTALLATION_FOLDER}"/bin/magento setup:install "${install_args[@]}"
