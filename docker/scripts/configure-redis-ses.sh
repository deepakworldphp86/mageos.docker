...
echo "Configuring Redis sessions: /var/www/html/${MAGE_INSTALLATION_FOLDER}/bin/magento setup:config:set --session-save=redis ${redis_ses_args[@]}"
php -d memory_limit=-1 /var/www/html/"${MAGE_INSTALLATION_FOLDER}"/bin/magento setup:config:set --session-save=redis "${redis_ses_args[@]}"
